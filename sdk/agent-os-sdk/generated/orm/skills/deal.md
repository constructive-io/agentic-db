...
db.deal.create({ data: { entityId: '<value>', name: '<value>', stage: '<value>', value: '<value>', notes: '<value>' }, select: { id: true } }).execute()
...
const item = await db.deal.create({
  data: { entityId: 'value', name: 'value', stage: 'value', value: 'value', notes: 'value' },
  select: { id: true }
}).execute();
```
