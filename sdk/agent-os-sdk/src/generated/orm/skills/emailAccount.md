...
db.emailAccount.create({ data: { entityId: '<value>', email: '<value>', provider: '<value>', syncState: '<value>' }, select: { id: true } }).execute()
...
const item = await db.emailAccount.create({
  data: { entityId: 'value', email: 'value', provider: 'value', syncState: 'value' },
  select: { id: true }
}).execute();
```
