...
db.project.create({ data: { entityId: '<value>', name: '<value>', description: '<value>', status: '<value>', startDate: '<value>', dueDate: '<value>', embedding: '<value>' }, select: { id: true } }).execute()
...
const item = await db.project.create({
  data: { entityId: 'value', name: 'value', description: 'value', status: 'value', startDate: 'value', dueDate: 'value', embedding: 'value' },
  select: { id: true }
}).execute();
```
