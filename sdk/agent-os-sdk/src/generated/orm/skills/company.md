...
db.company.create({ data: { entityId: '<value>', name: '<value>', domain: '<value>', industry: '<value>', description: '<value>', embedding: '<value>' }, select: { id: true } }).execute()
...
const item = await db.company.create({
  data: { entityId: 'value', name: 'value', domain: 'value', industry: 'value', description: 'value', embedding: 'value' },
  select: { id: true }
}).execute();
```
