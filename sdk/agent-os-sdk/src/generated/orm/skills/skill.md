...
db.skill.create({ data: { entityId: '<value>', name: '<value>', description: '<value>', content: '<value>', isActive: '<value>', embedding: '<value>' }, select: { id: true } }).execute()
...
const item = await db.skill.create({
  data: { entityId: 'value', name: 'value', description: 'value', content: 'value', isActive: 'value', embedding: 'value' },
  select: { id: true }
}).execute();
```
