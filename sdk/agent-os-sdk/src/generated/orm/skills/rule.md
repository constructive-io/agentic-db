...
db.rule.create({ data: { entityId: '<value>', title: '<value>', content: '<value>', kind: '<value>', isActive: '<value>', embedding: '<value>' }, select: { id: true } }).execute()
...
const item = await db.rule.create({
  data: { entityId: 'value', title: 'value', content: 'value', kind: 'value', isActive: 'value', embedding: 'value' },
  select: { id: true }
}).execute();
```
