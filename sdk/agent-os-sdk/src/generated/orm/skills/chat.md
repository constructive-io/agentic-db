...
db.chat.create({ data: { entityId: '<value>', title: '<value>', startedAt: '<value>', embedding: '<value>' }, select: { id: true } }).execute()
...
const item = await db.chat.create({
  data: { entityId: 'value', title: 'value', startedAt: 'value', embedding: 'value' },
  select: { id: true }
}).execute();
```
