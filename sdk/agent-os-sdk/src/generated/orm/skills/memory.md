...
db.memory.create({ data: { entityId: '<value>', content: '<value>', tags: '<value>', embedding: '<value>' }, select: { id: true } }).execute()
...
const item = await db.memory.create({
  data: { entityId: 'value', content: 'value', tags: 'value', embedding: 'value' },
  select: { id: true }
}).execute();
```
