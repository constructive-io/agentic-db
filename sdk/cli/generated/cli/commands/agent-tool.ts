...
import type { CreateAgentToolInput, AgentToolPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.agentTool
      .findMany({
        select: {
          agentId: true,
          toolId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(answers, fieldSchema) as CreateAgentToolInput['agentTool'];
    const client = getClient();
    const result = await client.agentTool
      .create({
        data: {
          agentId: cleanedData.agentId,
          toolId: cleanedData.toolId,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as AgentToolPatch;
    const client = getClient();
    const result = await client.agentTool
      .update({
        where: {
