...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    const defaultSelect = {
      ...
      intentTriggerEmbeddingUpdatedAt: true,
      agentId: true,
    };
    ...
    if (argv['auto-embed']) {
      ...
      findManyArgs.where = await autoEmbedWhere(
        findManyArgs.where ?? {},
        ['embedding', 'intentTriggerEmbedding'],
        embedder
      );
    }
    const client = getClient();
    const result = await client.skill.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<SkillSelect, SkillFilter, SkillOrderBy> & {
        select: SkillSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.skill.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
...
async function handleSearch(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    if (argv['auto-embed']) {
      ...
      await autoEmbedWhere(searchWhere ?? {}, ['embedding', 'intentTriggerEmbedding'], embedder);
    }
    const defaultSelect = {
      name: true,
      description: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    const rawAnswers = await prompter.prompt(argv, [
      ...
      {
        type: 'text',
        name: 'intentTriggerEmbeddingUpdatedAt',
        message: 'intentTriggerEmbeddingUpdatedAt',
        required: false,
        skipPrompt: true,
      },
      {
    ...
    if (argv['auto-embed']) {
      ...
      await autoEmbedInput(cleanedData, ['embedding', 'intentTriggerEmbedding'], embedder);
    }
    const client = getClient();
    const result = await client.skill
      .create({
        data: {
          ...
          intentTriggerEmbeddingUpdatedAt: cleanedData.intentTriggerEmbeddingUpdatedAt,
