/**
 * CLI commands for RuntimeLogChunk
...
import type { CreateRuntimeLogChunkInput, RuntimeLogChunkPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.runtimeLogChunk
      .findMany({
        select: {
          id: true,
          entityId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateRuntimeLogChunkInput['runtimeLogChunk'];
    const client = getClient();
    const result = await client.runtimeLogChunk
      .create({
        data: {
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as RuntimeLogChunkPatch;
    const client = getClient();
    const result = await client.runtimeLogChunk
      .update({
        where: {
