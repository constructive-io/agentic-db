/**
 * CLI commands for CalendarEventChunk
...
import type { CreateCalendarEventChunkInput, CalendarEventChunkPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.calendarEventChunk
      .findMany({
        select: {
          id: true,
          entityId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateCalendarEventChunkInput['calendarEventChunk'];
    const client = getClient();
    const result = await client.calendarEventChunk
      .create({
        data: {
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as CalendarEventChunkPatch;
    const client = getClient();
    const result = await client.calendarEventChunk
      .update({
        where: {
