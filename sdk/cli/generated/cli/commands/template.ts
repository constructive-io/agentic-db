...
import type { CreateTemplateInput, TemplatePatch } from '../../orm/input-types';
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(answers, fieldSchema) as CreateTemplateInput['template'];
    const client = getClient();
    const result = await client.template
      .create({
        data: {
          ...
          variables: cleanedData.variables,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as TemplatePatch;
    const client = getClient();
    const result = await client.template
      .update({
        where: {
