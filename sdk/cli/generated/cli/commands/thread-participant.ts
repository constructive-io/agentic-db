/**
 * CLI commands for ThreadParticipant
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.threadParticipant.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<ThreadParticipantSelect, ThreadParticipantFilter, ThreadParticipantOrderBy> & {
        select: ThreadParticipantSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.threadParticipant.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateThreadParticipantInput['threadParticipant'];
    const client = getClient();
    const result = await client.threadParticipant
      .create({
        data: {
