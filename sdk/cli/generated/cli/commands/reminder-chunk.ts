/**
 * CLI commands for ReminderChunk
...
import type { CreateReminderChunkInput, ReminderChunkPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.reminderChunk
      .findMany({
        select: {
          id: true,
          entityId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateReminderChunkInput['reminderChunk'];
    const client = getClient();
    const result = await client.reminderChunk
      .create({
        data: {
          ...
          reminderId: cleanedData.reminderId,
        },
        select: {
          id: true,
          entityId: true,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as ReminderChunkPatch;
    const client = getClient();
    const result = await client.reminderChunk
      .update({
        where: {
