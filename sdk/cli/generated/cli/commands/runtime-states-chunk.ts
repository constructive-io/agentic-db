/**
 * CLI commands for RuntimeStatesChunk
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.runtimeStatesChunk.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<RuntimeStatesChunkSelect, RuntimeStatesChunkFilter, never> & {
        select: RuntimeStatesChunkSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.runtimeStatesChunk.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
...
async function handleGet(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const client = getClient();
    const result = await client.runtimeStatesChunk
      .findOne({
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    const rawAnswers = await prompter.prompt(argv, [
      {
        type: 'text',
        name: 'runtimeStatesId',
        message: 'runtimeStatesId',
        required: true,
      },
      {
        type: 'text',
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateRuntimeStatesChunkInput['runtimeStatesChunk'];
    if (argv['auto-embed']) {
      const embedder = resolveEmbedder();
      if (!embedder) {
        console.error(
    ...
    const result = await client.runtimeStatesChunk
      .create({
        data: {
          runtimeStatesId: cleanedData.runtimeStatesId,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as RuntimeStatesChunkPatch;
    if (argv['auto-embed']) {
      const embedder = resolveEmbedder();
      if (!embedder) {
        console.error(
