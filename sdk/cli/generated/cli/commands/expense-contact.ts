...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<ExpenseContactSelect, ExpenseContactFilter, never, ExpenseContactOrderBy> & {
        select: ExpenseContactSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.expenseContact.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<ExpenseContactSelect, ExpenseContactFilter, never> & {
        select: ExpenseContactSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.expenseContact.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
