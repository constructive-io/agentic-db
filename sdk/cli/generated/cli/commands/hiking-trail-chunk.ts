/**
 * CLI commands for HikingTrailChunk
...
import type { CreateHikingTrailChunkInput, HikingTrailChunkPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.hikingTrailChunk
      .findMany({
        select: {
          id: true,
          entityId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateHikingTrailChunkInput['hikingTrailChunk'];
    const client = getClient();
    const result = await client.hikingTrailChunk
      .create({
        data: {
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as HikingTrailChunkPatch;
    const client = getClient();
    const result = await client.hikingTrailChunk
      .update({
        where: {
