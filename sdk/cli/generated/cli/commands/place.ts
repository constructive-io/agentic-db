...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<PlaceSelect, PlaceFilter, PlaceOrderBy> & {
        select: PlaceSelect;
      }
    >(argv, defaultSelect);
    if (argv['auto-embed']) {
    ...
    const result = await client.place.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<PlaceSelect, PlaceFilter, PlaceOrderBy> & {
        select: PlaceSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.place.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
