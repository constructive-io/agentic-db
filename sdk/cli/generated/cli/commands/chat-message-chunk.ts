/**
 * CLI commands for ChatMessageChunk
...
import type { CreateChatMessageChunkInput, ChatMessageChunkPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.chatMessageChunk
      .findMany({
        select: {
          id: true,
          entityId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateChatMessageChunkInput['chatMessageChunk'];
    const client = getClient();
    const result = await client.chatMessageChunk
      .create({
        data: {
          ...
          chatMessageId: cleanedData.chatMessageId,
        },
        select: {
          id: true,
          entityId: true,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as ChatMessageChunkPatch;
    const client = getClient();
    const result = await client.chatMessageChunk
      .update({
        where: {
