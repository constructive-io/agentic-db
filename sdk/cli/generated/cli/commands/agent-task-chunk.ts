/**
 * CLI commands for AgentTaskChunk
...
import type { CreateAgentTaskChunkInput, AgentTaskChunkPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.agentTaskChunk
      .findMany({
        select: {
          id: true,
          entityId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateAgentTaskChunkInput['agentTaskChunk'];
    const client = getClient();
    const result = await client.agentTaskChunk
      .create({
        data: {
          ...
          agentTaskId: cleanedData.agentTaskId,
        },
        select: {
          id: true,
          entityId: true,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as AgentTaskChunkPatch;
    const client = getClient();
    const result = await client.agentTaskChunk
      .update({
        where: {
