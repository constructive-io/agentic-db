/**
 * CLI commands for OrgProfileGrant
...
import type { CreateOrgProfileGrantInput, OrgProfileGrantPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.orgProfileGrant
      .findMany({
        select: {
          id: true,
          membershipId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateOrgProfileGrantInput['orgProfileGrant'];
    const client = getClient();
    const result = await client.orgProfileGrant
      .create({
        data: {
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as OrgProfileGrantPatch;
    const client = getClient();
    const result = await client.orgProfileGrant
      .update({
        where: {
