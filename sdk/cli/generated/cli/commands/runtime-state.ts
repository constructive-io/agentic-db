...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<RuntimeStateSelect, RuntimeStateFilter, RuntimeStateOrderBy> & {
    ...
    const result = await client.runtimeState.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<RuntimeStateSelect, RuntimeStateFilter, RuntimeStateOrderBy> & {
        select: RuntimeStateSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.runtimeState.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
