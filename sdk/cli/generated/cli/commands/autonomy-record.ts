...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.autonomyRecord.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<AutonomyRecordSelect, AutonomyRecordFilter, AutonomyRecordOrderBy> & {
        select: AutonomyRecordSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.autonomyRecord.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
