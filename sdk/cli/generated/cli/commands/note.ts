...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<NoteSelect, NoteFilter, never, NoteOrderBy> & {
        select: NoteSelect;
      }
    >(argv, defaultSelect);
    if (argv['auto-embed']) {
    ...
    const result = await client.note.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<NoteSelect, NoteFilter, never> & {
        select: NoteSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.note.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
