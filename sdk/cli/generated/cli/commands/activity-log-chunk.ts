/**
 * CLI commands for ActivityLogChunk
...
import type { CreateActivityLogChunkInput, ActivityLogChunkPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.activityLogChunk
      .findMany({
        select: {
          id: true,
          entityId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateActivityLogChunkInput['activityLogChunk'];
    const client = getClient();
    const result = await client.activityLogChunk
      .create({
        data: {
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as ActivityLogChunkPatch;
    const client = getClient();
    const result = await client.activityLogChunk
      .update({
        where: {
