/**
 * CLI commands for AutonomyRecordsChunk
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.autonomyRecordsChunk.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<AutonomyRecordsChunkSelect, AutonomyRecordsChunkFilter, never> & {
        select: AutonomyRecordsChunkSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.autonomyRecordsChunk.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateAutonomyRecordsChunkInput['autonomyRecordsChunk'];
    if (argv['auto-embed']) {
      const embedder = resolveEmbedder();
      if (!embedder) {
        console.error(
