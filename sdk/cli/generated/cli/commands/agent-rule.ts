...
import type { CreateAgentRuleInput, AgentRulePatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.agentRule
      .findMany({
        select: {
          agentId: true,
          ruleId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(answers, fieldSchema) as CreateAgentRuleInput['agentRule'];
    const client = getClient();
    const result = await client.agentRule
      .create({
        data: {
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as AgentRulePatch;
    const client = getClient();
    const result = await client.agentRule
      .update({
        where: {
