...
import type { CreateTripPlaceInput, TripPlacePatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.tripPlace
      .findMany({
        select: {
          tripId: true,
          placeId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(answers, fieldSchema) as CreateTripPlaceInput['tripPlace'];
    const client = getClient();
    const result = await client.tripPlace
      .create({
        data: {
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as TripPlacePatch;
    const client = getClient();
    const result = await client.tripPlace
      .update({
        where: {
