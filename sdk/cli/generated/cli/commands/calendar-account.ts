/**
 * CLI commands for CalendarAccount
...
import type { CreateCalendarAccountInput, CalendarAccountPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.calendarAccount
      .findMany({
        select: {
          id: true,
          entityId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateCalendarAccountInput['calendarAccount'];
    const client = getClient();
    const result = await client.calendarAccount
      .create({
        data: {
          ...
          syncToken: cleanedData.syncToken,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as CalendarAccountPatch;
    const client = getClient();
    const result = await client.calendarAccount
      .update({
        where: {
