/**
 * CLI commands for SessionArchiveChunk
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.sessionArchiveChunk
      .findMany({
        select: {
          id: true,
          entityId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    const rawAnswers = await prompter.prompt(argv, [
      ...
      {
        type: 'text',
        name: 'sessionArchiveId',
        message: 'sessionArchiveId',
        required: true,
      },
    ]);
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateSessionArchiveChunkInput['sessionArchiveChunk'];
    const client = getClient();
    const result = await client.sessionArchiveChunk
      .create({
        data: {
          ...
          sessionArchiveId: cleanedData.sessionArchiveId,
        },
        select: {
          id: true,
          entityId: true,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as SessionArchiveChunkPatch;
    const client = getClient();
    const result = await client.sessionArchiveChunk
      .update({
        where: {
