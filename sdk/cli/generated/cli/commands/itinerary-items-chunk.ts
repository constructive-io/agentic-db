/**
 * CLI commands for ItineraryItemsChunk
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.itineraryItemsChunk
      .findMany({
        select: {
          id: true,
          itineraryItemsId: true,
          content: true,
          chunkIndex: true,
          embedding: true,
          metadata: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    const rawAnswers = await prompter.prompt(argv, [
      {
        type: 'text',
        name: 'itineraryItemsId',
        message: 'itineraryItemsId',
        required: true,
      },
      {
        type: 'text',
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateItineraryItemsChunkInput['itineraryItemsChunk'];
    const client = getClient();
    const result = await client.itineraryItemsChunk
      .create({
        data: {
          itineraryItemsId: cleanedData.itineraryItemsId,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as ItineraryItemsChunkPatch;
    const client = getClient();
    const result = await client.itineraryItemsChunk
      .update({
        where: {
