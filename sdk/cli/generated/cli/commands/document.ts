...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<DocumentSelect, DocumentFilter, DocumentOrderBy> & {
    ...
    const result = await client.document.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<DocumentSelect, DocumentFilter, DocumentOrderBy> & {
        select: DocumentSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.document.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    ...
    const result = await client.document
      .create({
        data: {
          ...
          repoName: cleanedData.repoName,
          filePath: cleanedData.filePath,
          commitHash: cleanedData.commitHash,
