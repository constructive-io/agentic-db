/**
 * CLI commands for RepositoryChunk
...
import type { CreateRepositoryChunkInput, RepositoryChunkPatch } from '../../orm/input-types';
...
async function handleList(_argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.repositoryChunk
      .findMany({
        select: {
          id: true,
          entityId: true,
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateRepositoryChunkInput['repositoryChunk'];
    const client = getClient();
    const result = await client.repositoryChunk
      .create({
        data: {
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as RepositoryChunkPatch;
    const client = getClient();
    const result = await client.repositoryChunk
      .update({
        where: {
