...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.toolDefinition.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<ToolDefinitionSelect, ToolDefinitionFilter, ToolDefinitionOrderBy> & {
        select: ToolDefinitionSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.toolDefinition.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
