...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<TaskNoteSelect, TaskNoteFilter, never, TaskNoteOrderBy> & {
        select: TaskNoteSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.taskNote.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<TaskNoteSelect, TaskNoteFilter, never> & {
        select: TaskNoteSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.taskNote.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
