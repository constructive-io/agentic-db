...
db.venue.create({ data: { entityId: '<value>', name: '<value>', neighborhood: '<value>', city: '<value>', status: '<value>', notes: '<value>' }, select: { id: true } }).execute()
...
const item = await db.venue.create({
  data: { entityId: 'value', name: 'value', neighborhood: 'value', city: 'value', status: 'value', notes: 'value' },
  select: { id: true }
}).execute();
```
