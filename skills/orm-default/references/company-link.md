...
db.companyLink.create({ data: { entityId: '<value>', title: '<value>', url: '<value>', embedding: '<value>', companyId: '<value>', embeddingDistance: '<value>' }, select: { id: true } }).execute()
...
const item = await db.companyLink.create({
  data: { entityId: 'value', title: 'value', url: 'value', embedding: 'value', companyId: 'value', embeddingDistance: 'value' },
  select: { id: true }
}).execute();
```
