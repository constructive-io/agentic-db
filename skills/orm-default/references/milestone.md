...
db.milestone.create({ data: { entityId: '<value>', projectId: '<value>', name: '<value>', dueDate: '<value>', status: '<value>' }, select: { id: true } }).execute()
...
const item = await db.milestone.create({
  data: { entityId: 'value', projectId: 'value', name: 'value', dueDate: 'value', status: 'value' },
  select: { id: true }
}).execute();
```
