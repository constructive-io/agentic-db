...
db.blueprint.create({ data: { entityId: '<value>', title: '<value>', steps: '<value>', triggerConditions: '<value>', embedding: '<value>', embeddingDistance: '<value>' }, select: { id: true } }).execute()
...
const item = await db.blueprint.create({
  data: { entityId: 'value', title: 'value', steps: 'value', triggerConditions: 'value', embedding: 'value', embeddingDistance: 'value' },
  select: { id: true }
}).execute();
```
