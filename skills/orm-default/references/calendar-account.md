...
db.calendarAccount.create({ data: { entityId: '<value>', email: '<value>', provider: '<value>', syncToken: '<value>', lastSyncedAt: '<value>' }, select: { id: true } }).execute()
...
const item = await db.calendarAccount.create({
  data: { entityId: 'value', email: 'value', provider: 'value', syncToken: 'value', lastSyncedAt: 'value' },
  select: { id: true }
}).execute();
```
