-- Deploy: schemas/agent-os-1773551593867-bac64076-auth-public/procedures/verify_email/procedure
...
CREATE FUNCTION "agent-os-1773551593867-bac64076-auth-public".verify_email (
  email_id uuid,
  token text
)
  RETURNS boolean
...
BEGIN
  ...
  verification_email_sent_at = "agent-os-1773551593867-bac64076-simple-secrets".get(v_user_id, 'verification_email_sent_at');
  IF (verification_email_sent_at IS NOT NULL AND 
    verification_email_sent_at + v_verification_expires_interval < NOW() 
  ) THEN 
    
    PERFORM "agent-os-1773551593867-bac64076-simple-secrets".del(v_user_id, ARRAY[
        'verification_email_sent_at',
        'verification_email_attempts',
        'first_failed_verification_email_attempt'
    ]);
    PERFORM "agent-os-1773551593867-bac64076-encrypted".del(v_user_id, verification_token_name);
    RETURN FALSE;
  END IF;
  verification_token_name = v_email.email::text || '_verification_token';
  IF ("agent-os-1773551593867-bac64076-encrypted".verify (v_user_id, verification_token_name, verify_email.token) ) THEN
    UPDATE "agent-os-1773551593867-bac64076-user-identifiers-public".emails e
  ...
  ELSE
    ...
    END IF;
    PERFORM "agent-os-1773551593867-bac64076-simple-secrets".set(v_user_id, 'verification_email_attempts', verification_email_attempts);
    PERFORM "agent-os-1773551593867-bac64076-simple-secrets".set(v_user_id, 'first_failed_verification_email_attempt', first_failed_verification_email_attempt);
    RETURN FALSE;
  END IF;
END;
$$
...
VOLATILE
SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION "agent-os-1773551593867-bac64076-auth-public".verify_email TO anonymous, authenticated;
