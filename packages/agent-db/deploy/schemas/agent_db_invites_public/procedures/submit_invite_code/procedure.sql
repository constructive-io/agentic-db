-- Deploy: schemas/agent_db_invites_public/procedures/submit_invite_code/procedure
-- made with <3 @ constructive.io

-- requires: schemas/agent_db_invites_public/schema
-- requires: schemas/agent_db_memberships_public/tables/app_memberships/table
-- requires: schemas/agent_db_memberships_private/procedures/app_memberships_perm_check/procedure



CREATE FUNCTION agent_db_invites_public.submit_invite_code (
  token text
)
  RETURNS boolean
  AS $$
DECLARE
  ...
  v_invite agent_db_invites_public.invites;
BEGIN
  SELECT * FROM agent_db_users_public.users
  ...
  SELECT * FROM agent_db_invites_public.invites i
  ...
  IF (v_email.email IS NOT NULL OR v_invite.multiple IS FALSE) THEN 
    UPDATE agent_db_invites_public.invites 
  ...
  WHERE id = v_invite.id;
  INSERT INTO agent_db_invites_public.claimed_invites 
    (sender_id, receiver_id, data)
  VALUES (v_invite.sender_id, v_user.id, v_invite.data);
  IF (
    agent_db_memberships_private.app_memberships_perm_check(
        'send_approved_invites',
        v_invite.sender_id
    ) IS TRUE
   ) THEN
    INSERT INTO agent_db_memberships_public.app_memberships 
...
VOLATILE
SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION agent_db_invites_public.submit_invite_code TO authenticated;
