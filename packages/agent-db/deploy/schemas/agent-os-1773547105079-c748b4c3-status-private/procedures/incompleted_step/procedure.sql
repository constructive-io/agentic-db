-- Deploy: schemas/agent-os-1773547105079-c748b4c3-status-private/procedures/incompleted_step/procedure
...
CREATE FUNCTION "agent-os-1773547105079-c748b4c3-status-private".incompleted_step (
  step text,
  actor_id uuid DEFAULT jwt_public.current_user_id()
) RETURNS void AS $EOFCODE$
BEGIN
  IF (incompleted_step.actor_id IS NOT NULL) THEN 
    DELETE FROM "agent-os-1773547105079-c748b4c3-status-public".app_steps s
      WHERE s.actor_id = incompleted_step.actor_id
      AND s.name = step;
    DELETE FROM "agent-os-1773547105079-c748b4c3-status-public".app_achievements a
      WHERE a.actor_id = incompleted_step.actor_id
      AND a.name = step;
  END IF;
END;
$EOFCODE$ LANGUAGE plpgsql VOLATILE SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION "agent-os-1773547105079-c748b4c3-status-private".incompleted_step TO authenticated;
