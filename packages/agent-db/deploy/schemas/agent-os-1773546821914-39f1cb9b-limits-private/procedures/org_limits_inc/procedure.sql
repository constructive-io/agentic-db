-- Deploy: schemas/agent-os-1773546821914-39f1cb9b-limits-private/procedures/org_limits_inc/procedure
...
CREATE FUNCTION "agent-os-1773546821914-39f1cb9b-limits-private".org_limits_inc (
    limitname citext,
    entity_id uuid,
...
BEGIN
    SELECT EXISTS (SELECT 1 FROM "agent-os-1773546821914-39f1cb9b-limits-public".org_limits l
    ...
    IF (limit_exists IS FALSE) THEN 
        ...
        IF (NOT FOUND) THEN 
            max_default = 0;
        END IF;
        INSERT INTO "agent-os-1773546821914-39f1cb9b-limits-public".org_limits (name, num, max, actor_id, entity_id)
        VALUES (limitname, 0, max_default, actor_id, entity_id);
    END IF;
    SELECT * FROM "agent-os-1773546821914-39f1cb9b-limits-public".org_limits l
        WHERE l.name = limitname
        AND l.actor_id = org_limits_inc.actor_id
        AND l.entity_id = org_limits_inc.entity_id
    INTO rec;
    IF (rec.max < 0 OR rec.max >= rec.num + amount) THEN 
        UPDATE "agent-os-1773546821914-39f1cb9b-limits-public".org_limits l
        SET num = num + amount
        WHERE l.name = limitname
        AND l.actor_id = org_limits_inc.actor_id
        AND l.entity_id = org_limits_inc.entity_id;
