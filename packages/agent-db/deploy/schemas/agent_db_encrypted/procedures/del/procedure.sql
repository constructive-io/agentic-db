-- Deploy: schemas/agent_db_encrypted/procedures/del/procedure
...
CREATE FUNCTION agent_db_encrypted.del(
  owner_id uuid,
  secret_name text
)
  RETURNS void
  AS $$
BEGIN
  DELETE FROM agent_db_encrypted.encrypted_secrets s
...
LANGUAGE 'plpgsql'
VOLATILE;
GRANT EXECUTE ON FUNCTION agent_db_encrypted.del(uuid,text) TO authenticated;
GRANT EXECUTE ON FUNCTION agent_db_encrypted.del(uuid,text[]) TO authenticated;
