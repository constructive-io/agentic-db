...
CREATE FUNCTION agent_db_auth_public.sign_in_one_time_token (
  token text,
...
BEGIN
    ...
    FROM agent_db_auth_private.session_credentials c
    JOIN agent_db_auth_private.sessions s ON s.id = c.session_id
...
LANGUAGE 'plpgsql'
SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION agent_db_auth_public.sign_in_one_time_token TO anonymous;
