-- Deploy: schemas/agent-os-1773550873753-b6c4a3e1-auth-public/procedures/sign_in/procedure
...
CREATE FUNCTION "agent-os-1773550873753-b6c4a3e1-auth-public".sign_in (
  email text,
  password text,
...
DECLARE
  ...
  v_email "agent-os-1773550873753-b6c4a3e1-user-identifiers-public".emails;
  v_settings "agent-os-1773550873753-b6c4a3e1-auth-private".app_auth_settings;
  ...
  v_csrf_secret text;
  v_anon_session "agent-os-1773550873753-b6c4a3e1-auth-private".sessions;
  
  v_session_expires_at timestamptz;
BEGIN
  SELECT * FROM "agent-os-1773550873753-b6c4a3e1-auth-private".app_auth_settings LIMIT 1 INTO v_settings;
  ...
  IF (csrf_token IS NOT NULL) THEN
    SELECT s.*
    FROM "agent-os-1773550873753-b6c4a3e1-auth-private".sessions AS s
  ...
  SELECT
    user_emails_alias.*
  FROM
    "agent-os-1773550873753-b6c4a3e1-user-identifiers-public".emails AS user_emails_alias
  WHERE
  ...
  IF (NOT FOUND) THEN
    RETURN;
  END IF;
  first_failed_password_attempt = "agent-os-1773550873753-b6c4a3e1-simple-secrets".get(v_email.owner_id, 'first_failed_password_attempt');
  password_attempts = "agent-os-1773550873753-b6c4a3e1-simple-secrets".get(v_email.owner_id, 'password_attempts');
  IF (
  ...
  END IF;
  IF ( "agent-os-1773550873753-b6c4a3e1-encrypted".verify(v_email.owner_id, 'password_hash', PASSWORD) ) THEN
    PERFORM "agent-os-1773550873753-b6c4a3e1-simple-secrets".del(v_email.owner_id,
    ARRAY[
    ...
    IF (v_anon_session.id IS NOT NULL) THEN
      UPDATE "agent-os-1773550873753-b6c4a3e1-auth-private".sessions
        SET revoked_at = NOW()
    ...
    END IF;
    INSERT INTO "agent-os-1773550873753-b6c4a3e1-auth-private".sessions (
      id,
      user_id,
      is_anonymous,
      expires_at,
    ...
    INSERT INTO "agent-os-1773550873753-b6c4a3e1-auth-private".session_credentials (
      id,
      session_id,
      kind,
      secret_hash,
  ...
  ELSE
   INSERT INTO "agent-os-1773550873753-b6c4a3e1-logging-public".audit_logs 
    ...
    END IF;
    PERFORM "agent-os-1773550873753-b6c4a3e1-simple-secrets".set(v_email.owner_id, 'password_attempts', password_attempts);
    PERFORM "agent-os-1773550873753-b6c4a3e1-simple-secrets".set(v_email.owner_id, 'first_failed_password_attempt', first_failed_password_attempt);
    RETURN;
  END IF;
END;
$$
LANGUAGE 'plpgsql'
SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION "agent-os-1773550873753-b6c4a3e1-auth-public".sign_in TO anonymous;
