-- Deploy: schemas/agent-os-1773550873753-b6c4a3e1-auth-public/procedures/one_time_token/procedure
...
CREATE FUNCTION "agent-os-1773550873753-b6c4a3e1-auth-public".one_time_token (
  email text,
  password text,
...
DECLARE
  ...
  v_ot_token text;
BEGIN
  SELECT si.id FROM "agent-os-1773550873753-b6c4a3e1-auth-public".sign_in(
    email, password, remember_me
  ) AS si INTO v_credential_id;
  IF (v_credential_id IS NULL) THEN 
    RETURN NULL;
  END IF;
  SELECT c.session_id FROM "agent-os-1773550873753-b6c4a3e1-auth-private".session_credentials c
...
LANGUAGE 'plpgsql'
SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION "agent-os-1773550873753-b6c4a3e1-auth-public".one_time_token TO anonymous;
