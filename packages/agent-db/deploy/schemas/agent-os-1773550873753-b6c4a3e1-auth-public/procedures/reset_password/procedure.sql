-- Deploy: schemas/agent-os-1773550873753-b6c4a3e1-auth-public/procedures/reset_password/procedure
...
CREATE FUNCTION "agent-os-1773550873753-b6c4a3e1-auth-public".reset_password (role_id uuid, reset_token text, new_password text)
    RETURNS boolean
AS $$
DECLARE
...
BEGIN
    ...
    SELECT  
        is_disabled,
        is_banned
        FROM "agent-os-1773550873753-b6c4a3e1-memberships-public".app_memberships
    ...
    END IF;
    reset_password_attempts = "agent-os-1773550873753-b6c4a3e1-simple-secrets".get(v_user.id, 'reset_password_attempts', '0');
    first_failed_reset_password_attempt = "agent-os-1773550873753-b6c4a3e1-simple-secrets".get(v_user.id, 'first_failed_reset_password_attempt');
    ...
    END IF;
    IF ("agent-os-1773550873753-b6c4a3e1-encrypted".verify(v_user.id, 'reset_password_token', reset_token)) THEN
        PERFORM "agent-os-1773550873753-b6c4a3e1-encrypted".set
            (v_user.id, 'password_hash', new_password, 'crypt');
        PERFORM "agent-os-1773550873753-b6c4a3e1-simple-secrets".del(
            v_user.id,
            ARRAY[
                'password_attempts',
        ...
        PERFORM "agent-os-1773550873753-b6c4a3e1-encrypted".del(
            v_user.id,
    ...
    ELSE
        ...
        END IF;
        PERFORM "agent-os-1773550873753-b6c4a3e1-simple-secrets".set(v_user.id, 'reset_password_attempts', reset_password_attempts);
        PERFORM "agent-os-1773550873753-b6c4a3e1-simple-secrets".set(v_user.id, 'first_failed_reset_password_attempt', first_failed_reset_password_attempt);
    END IF;
    RETURN FALSE;
END;
$$
...
SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION "agent-os-1773550873753-b6c4a3e1-auth-public".reset_password to anonymous;
REVOKE EXECUTE ON FUNCTION "agent-os-1773550873753-b6c4a3e1-auth-public".reset_password from authenticated;
