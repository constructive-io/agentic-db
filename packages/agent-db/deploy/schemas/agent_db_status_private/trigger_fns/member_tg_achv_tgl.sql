-- Deploy: schemas/agent_db_status_private/trigger_fns/member_tg_achv_tgl
...
-- requires: schemas/agent_db_status_private/procedures/member_incompleted_step/procedure



CREATE FUNCTION "agent_db_status_private".member_tg_achv_tgl ()
  RETURNS TRIGGER
  AS $$
DECLARE
  is_null boolean;
...
BEGIN
    IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
        ...
        IF (is_null IS TRUE) THEN
            PERFORM "agent_db_status_private".member_incompleted_step(task_name, entity_id);
        ELSE
...
LANGUAGE 'plpgsql'
VOLATILE;
GRANT EXECUTE ON FUNCTION "agent_db_status_private".member_tg_achv_tgl TO authenticated;
