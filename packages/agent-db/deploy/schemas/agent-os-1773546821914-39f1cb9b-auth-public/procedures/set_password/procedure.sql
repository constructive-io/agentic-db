-- Deploy: schemas/agent-os-1773546821914-39f1cb9b-auth-public/procedures/set_password/procedure
...
CREATE FUNCTION "agent-os-1773546821914-39f1cb9b-auth-public".set_password (
...
DECLARE
  ...
  v_user_secret "agent-os-1773546821914-39f1cb9b-simple-secrets".secrets;
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1
      FROM "agent-os-1773546821914-39f1cb9b-encrypted".encrypted_secrets
      WHERE owner_id=v_user.id
        AND name='password_hash'
  )
  INTO password_exists;
  IF (password_exists IS TRUE) THEN 
    IF ("agent-os-1773546821914-39f1cb9b-encrypted".verify(
        v_user.id,
        'password_hash',
        current_password
    ) IS FALSE) THEN 
...
GRANT EXECUTE ON FUNCTION "agent-os-1773546821914-39f1cb9b-auth-public".set_password TO authenticated;
