...
CREATE FUNCTION "agent_db_limits_private".org_limits_check (limitname citext, amount int default 1, user_id uuid default jwt_public.current_user_id())
  RETURNS BOOLEAN
AS $CODEZ$
DECLARE
...
BEGIN
    SELECT EXISTS (SELECT 1 FROM "agent_db_limits_public".org_limits 
    ...
    IF (limit_exists IS FALSE) THEN 
        ...
        IF (NOT FOUND) THEN 
            max_default = 0;
        END IF;
        INSERT INTO "agent_db_limits_public".org_limits (name, num, max, actor_id)
        VALUES (limitname, 0, max_default, user_id);
    END IF;
    SELECT * FROM "agent_db_limits_public".org_limits
        WHERE name = limitname
        AND actor_id = user_id
    INTO rec;
