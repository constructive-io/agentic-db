-- Deploy: schemas/agent_db_memberships_private/trigger_fns/membership_mbr_create
...
-- requires: schemas/agent_db_permissions_public/procedures/org_permissions_get_mask_by_names/procedure



CREATE FUNCTION "agent_db_memberships_private".membership_mbr_create ()
  RETURNS TRIGGER
AS $CODEZ$
DECLARE
...
BEGIN
    ...
    ELSEIF (owner_id IS NOT NULL) THEN 
        ...
        INSERT INTO "agent_db_memberships_public".org_membership_defaults
        ( entity_id  ) -- is_approved, etc., defaults from table itself
        VALUES ( NEW.id );
        INSERT INTO "agent_db_permissions_public".org_permission_defaults 
            (permissions, entity_id)
        VALUES 
        (
            "agent_db_permissions_public".org_permissions_get_mask_by_names(
