-- Deploy: schemas/agent-os-1773550873753-b6c4a3e1-encrypted/procedures/verify/procedure
...
CREATE FUNCTION "agent-os-1773550873753-b6c4a3e1-encrypted".verify (
  owner_id uuid,
  secret_name text,
  value text
)
...
DECLARE
  v_secret_text text;
  v_secret "agent-os-1773550873753-b6c4a3e1-encrypted".encrypted_secrets;
BEGIN
  SELECT
    *
  FROM
    "agent-os-1773550873753-b6c4a3e1-encrypted".get (verify.owner_id, verify.secret_name)
  INTO v_secret_text;
  SELECT
    *
  FROM
    "agent-os-1773550873753-b6c4a3e1-encrypted".encrypted_secrets s
  WHERE
...
LANGUAGE 'plpgsql'
STABLE;
GRANT EXECUTE ON FUNCTION "agent-os-1773550873753-b6c4a3e1-encrypted".verify TO authenticated;
