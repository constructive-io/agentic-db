-- Deploy: schemas/agent-os-1773547105079-c748b4c3-status-public/procedures/steps_required/procedure
...
CREATE FUNCTION "agent-os-1773547105079-c748b4c3-status-public".steps_required (
    vlevel text,
    vrole_id uuid DEFAULT jwt_public.current_user_id()
)
  RETURNS SETOF "agent-os-1773547105079-c748b4c3-status-public".app_level_requirements
AS $CODEZ$
BEGIN
  RETURN QUERY
  SELECT 
    ...
    FROM
      "agent-os-1773547105079-c748b4c3-status-public".app_level_requirements 
    FULL OUTER JOIN "agent-os-1773547105079-c748b4c3-status-public".app_achievements ON (
      app_achievements.name = app_level_requirements.name
      AND app_achievements.actor_id = vrole_id
    )	
    JOIN "agent-os-1773547105079-c748b4c3-status-public".app_levels ON (app_level_requirements.level = app_levels.name)
  WHERE
...
GRANT EXECUTE ON FUNCTION "agent-os-1773547105079-c748b4c3-status-public".steps_required TO authenticated;
