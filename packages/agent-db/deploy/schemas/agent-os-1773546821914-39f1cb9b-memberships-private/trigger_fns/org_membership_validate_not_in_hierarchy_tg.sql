-- Deploy: schemas/agent-os-1773546821914-39f1cb9b-memberships-private/trigger_fns/org_membership_validate_not_in_hierarchy_tg
-- made with <3 @ launchql.com

-- requires: schemas/agent-os-1773546821914-39f1cb9b-memberships-private/schema


CREATE FUNCTION "agent-os-1773546821914-39f1cb9b-memberships-private".org_membership_validate_not_in_hierarchy_tg() RETURNS TRIGGER AS $_PGFN_$
                ...
                BEGIN
                    -- Calculate old and new is_active status
                    ...
                    -- Only check if transitioning from active to inactive
                    IF (v_old_is_active IS TRUE AND v_new_is_active IS NOT TRUE) THEN
                        -- Check if user is in the hierarchy for this entity
                        SELECT EXISTS (
                            SELECT 1
                            FROM "agent-os-1773546821914-39f1cb9b-memberships-public".org_chart_edges e
                            WHERE e.entity_id = NEW.entity_id
                                AND (e.child_id = NEW.actor_id OR e.parent_id = NEW.actor_id)
                        ) INTO v_in_hierarchy;

                        IF (v_in_hierarchy) THEN
                            RAISE EXCEPTION 'HIERARCHY_MEMBER_IN_USE: Cannot deactivate user % - user must be removed from the organization hierarchy first',
                                NEW.actor_id;
                        END IF;
                    END IF;
