...
BEGIN
  ...
  IF (verification_email_sent_at IS NOT NULL AND 
    ...
    PERFORM "agentic_db_simple_secrets".del(v_user_id, ARRAY[
  ...
  IF ("agentic_db_encrypted".verify (v_user_id, verification_token_name, verify_email.token) ) THEN
    UPDATE "agentic_db_user_identifiers_public".emails e
...
VOLATILE
SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION "agentic_db_auth_public".verify_email TO anonymous, authenticated;
