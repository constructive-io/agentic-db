-- Deploy: schemas/agent_db_auth_public/procedures/sign_in/procedure
...
CREATE FUNCTION "agent_db_auth_public".sign_in (
  email text,
  password text,
...
BEGIN
  ...
  SELECT
    user_emails_alias.*
  FROM
    "agent_db_user_identifiers_public".emails AS user_emails_alias
  WHERE
  ...
  IF (NOT FOUND) THEN
    RETURN;
  END IF;
  first_failed_password_attempt = "agent_db_simple_secrets".get(v_email.owner_id, 'first_failed_password_attempt');
  password_attempts = "agent_db_simple_secrets".get(v_email.owner_id, 'password_attempts');
  IF (
  ...
  SELECT  
      ...
      mem.is_disabled,
      mem.is_banned
    FROM "agent_db_memberships_public".app_memberships AS mem
  ...
  END IF;
  IF ( "agent_db_encrypted".verify(v_email.owner_id, 'password_hash', PASSWORD) ) THEN
    PERFORM "agent_db_simple_secrets".del(v_email.owner_id,
    ARRAY[
  ...
  ELSE
   INSERT INTO "agent_db_logging_public".audit_logs 
    ...
    END IF;
    PERFORM "agent_db_simple_secrets".set(v_email.owner_id, 'password_attempts', password_attempts);
    PERFORM "agent_db_simple_secrets".set(v_email.owner_id, 'first_failed_password_attempt', first_failed_password_attempt);
    RETURN;
  END IF;
END;
$$
LANGUAGE 'plpgsql'
SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION "agent_db_auth_public".sign_in TO anonymous;
