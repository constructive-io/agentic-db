...
// Get one by id
const item = await db.orgGetManagersRecord.findOne({ where: { id: '<value>' }, select: { userId: true, depth: true } }).execute();
...
// Get one by id
const item = await db.orgGetSubordinatesRecord.findOne({ where: { id: '<value>' }, select: { userId: true, depth: true } }).execute();
...
// Get one by id
const item = await db.appProfilePermission.findOne({ where: { id: '<value>' }, select: { id: true, profileId: true, permissionId: true, createdAt: true, updatedAt: true } }).execute();

// Create
const created = await db.appProfilePermission.create({ data: { profileId: '<value>', permissionId: '<value>' }, select: { id: true } }).execute();

// Update
const updated = await db.appProfilePermission.update({ where: { id: '<value>' }, data: { profileId: '<new-value>' }, select: { id: true } }).execute();

// Delete
const deleted = await db.appProfilePermission.delete({ where: { id: '<value>' } }).execute();
```
...
// Get one by id
const item = await db.appMembership.findOne({ where: { id: '<value>' }, select: { id: true, createdAt: true, updatedAt: true, createdBy: true, updatedBy: true, isApproved: true, isBanned: true, isDisabled: true, isVerified: true, isActive: true, isOwner: true, isAdmin: true, permissions: true, granted: true, actorId: true, profileId: true } }).execute();
...
// Get one by id
const item = await db.tableModule.findOne({ where: { id: '<value>' }, select: { id: true, databaseId: true, schemaId: true, tableId: true, tableName: true, nodeType: true, useRls: true, data: true, fields: true } }).execute();
...
const items = await db.secureTableProvision.findMany({ select: { id: true, databaseId: true, schemaId: true, tableId: true, tableName: true, nodeType: true, useRls: true, nodeData: true, grantRoles: true, grantPrivileges: true, policyType: true, policyPrivileges: true, policyRole: true, policyPermissive: true, policyData: true, outFields: true } }).execute();

// Get one by id
const item = await db.secureTableProvision.findOne({ where: { id: '<value>' }, select: { id: true, databaseId: true, schemaId: true, tableId: true, tableName: true, nodeType: true, useRls: true, nodeData: true, grantRoles: true, grantPrivileges: true, policyType: true, policyPrivileges: true, policyRole: true, policyPermissive: true, policyData: true, outFields: true } }).execute();
...
const items = await db.relationProvision.findMany({ select: { id: true, databaseId: true, relationType: true, sourceTableId: true, targetTableId: true, fieldName: true, deleteAction: true, isRequired: true, junctionTableId: true, junctionTableName: true, junctionSchemaId: true, sourceFieldName: true, targetFieldName: true, useCompositeKey: true, nodeType: true, nodeData: true, grantRoles: true, grantPrivileges: true, policyType: true, policyPrivileges: true, policyRole: true, policyPermissive: true, policyData: true, outFieldId: true, outJunctionTableId: true, outSourceFieldId: true, outTargetFieldId: true } }).execute();

// Get one by id
const item = await db.relationProvision.findOne({ where: { id: '<value>' }, select: { id: true, databaseId: true, relationType: true, sourceTableId: true, targetTableId: true, fieldName: true, deleteAction: true, isRequired: true, junctionTableId: true, junctionTableName: true, junctionSchemaId: true, sourceFieldName: true, targetFieldName: true, useCompositeKey: true, nodeType: true, nodeData: true, grantRoles: true, grantPrivileges: true, policyType: true, policyPrivileges: true, policyRole: true, policyPermissive: true, policyData: true, outFieldId: true, outJunctionTableId: true, outSourceFieldId: true, outTargetFieldId: true } }).execute();
...
// Get one by id
const item = await db.orgMembership.findOne({ where: { id: '<value>' }, select: { id: true, createdAt: true, updatedAt: true, createdBy: true, updatedBy: true, isApproved: true, isBanned: true, isDisabled: true, isActive: true, isOwner: true, isAdmin: true, permissions: true, granted: true, actorId: true, entityId: true, profileId: true } }).execute();
...
// Get one by id
const item = await db.orgProfilePermission.findOne({ where: { id: '<value>' }, select: { id: true, profileId: true, permissionId: true, createdAt: true, updatedAt: true } }).execute();

// Create
const created = await db.orgProfilePermission.create({ data: { profileId: '<value>', permissionId: '<value>' }, select: { id: true } }).execute();

// Update
const updated = await db.orgProfilePermission.update({ where: { id: '<value>' }, data: { profileId: '<new-value>' }, select: { id: true } }).execute();

// Delete
const deleted = await db.orgProfilePermission.delete({ where: { id: '<value>' } }).execute();
```
...
// Get one by id
const item = await db.orgProfileDefinitionGrant.findOne({ where: { id: '<value>' }, select: { id: true, profileId: true, permissionId: true, grantorId: true, isGrant: true, createdAt: true, updatedAt: true } }).execute();

// Create
const created = await db.orgProfileDefinitionGrant.create({ data: { profileId: '<value>', permissionId: '<value>', grantorId: '<value>', isGrant: '<value>' }, select: { id: true } }).execute();

// Update
const updated = await db.orgProfileDefinitionGrant.update({ where: { id: '<value>' }, data: { profileId: '<new-value>' }, select: { id: true } }).execute();

// Delete
const deleted = await db.orgProfileDefinitionGrant.delete({ where: { id: '<value>' } }).execute();
```
...
// Get one by id
const item = await db.orgProfileGrant.findOne({ where: { id: '<value>' }, select: { id: true, membershipId: true, profileId: true, entityId: true, grantorId: true, isGrant: true, createdAt: true, updatedAt: true } }).execute();

// Create
const created = await db.orgProfileGrant.create({ data: { membershipId: '<value>', profileId: '<value>', entityId: '<value>', grantorId: '<value>', isGrant: '<value>' }, select: { id: true } }).execute();

// Update
const updated = await db.orgProfileGrant.update({ where: { id: '<value>' }, data: { membershipId: '<new-value>' }, select: { id: true } }).execute();

// Delete
const deleted = await db.orgProfileGrant.delete({ where: { id: '<value>' } }).execute();
```
...
// Get one by id
const item = await db.orgChartEdge.findOne({ where: { id: '<value>' }, select: { id: true, createdAt: true, updatedAt: true, entityId: true, childId: true, parentId: true, positionTitle: true, positionLevel: true } }).execute();
...
// Get one by id
const item = await db.orgChartEdgeGrant.findOne({ where: { id: '<value>' }, select: { id: true, entityId: true, childId: true, parentId: true, grantorId: true, isGrant: true, positionTitle: true, positionLevel: true, createdAt: true } }).execute();
...
// Get one by id
const item = await db.appProfileGrant.findOne({ where: { id: '<value>' }, select: { id: true, membershipId: true, profileId: true, grantorId: true, isGrant: true, createdAt: true, updatedAt: true } }).execute();

// Create
const created = await db.appProfileGrant.create({ data: { membershipId: '<value>', profileId: '<value>', grantorId: '<value>', isGrant: '<value>' }, select: { id: true } }).execute();

// Update
const updated = await db.appProfileGrant.update({ where: { id: '<value>' }, data: { membershipId: '<new-value>' }, select: { id: true } }).execute();

// Delete
const deleted = await db.appProfileGrant.delete({ where: { id: '<value>' } }).execute();
```
...
// Get one by id
const item = await db.appProfileDefinitionGrant.findOne({ where: { id: '<value>' }, select: { id: true, profileId: true, permissionId: true, grantorId: true, isGrant: true, createdAt: true, updatedAt: true } }).execute();

// Create
const created = await db.appProfileDefinitionGrant.create({ data: { profileId: '<value>', permissionId: '<value>', grantorId: '<value>', isGrant: '<value>' }, select: { id: true } }).execute();

// Update
const updated = await db.appProfileDefinitionGrant.update({ where: { id: '<value>' }, data: { profileId: '<new-value>' }, select: { id: true } }).execute();

// Delete
const deleted = await db.appProfileDefinitionGrant.delete({ where: { id: '<value>' } }).execute();
```
...
// Get one by id
const item = await db.orgProfile.findOne({ where: { id: '<value>' }, select: { id: true, name: true, slug: true, description: true, permissions: true, isSystem: true, isDefault: true, createdAt: true, updatedAt: true, entityId: true } }).execute();

// Create
const created = await db.orgProfile.create({ data: { name: '<value>', slug: '<value>', description: '<value>', permissions: '<value>', isSystem: '<value>', isDefault: '<value>', entityId: '<value>' }, select: { id: true } }).execute();

// Update
const updated = await db.orgProfile.update({ where: { id: '<value>' }, data: { name: '<new-value>' }, select: { id: true } }).execute();

// Delete
const deleted = await db.orgProfile.delete({ where: { id: '<value>' } }).execute();
```
...
// Get one by id
const item = await db.appProfile.findOne({ where: { id: '<value>' }, select: { id: true, name: true, slug: true, description: true, permissions: true, isSystem: true, isDefault: true, createdAt: true, updatedAt: true } }).execute();

// Create
const created = await db.appProfile.create({ data: { name: '<value>', slug: '<value>', description: '<value>', permissions: '<value>', isSystem: '<value>', isDefault: '<value>' }, select: { id: true } }).execute();

// Update
const updated = await db.appProfile.update({ where: { id: '<value>' }, data: { name: '<new-value>' }, select: { id: true } }).execute();

// Delete
const deleted = await db.appProfile.delete({ where: { id: '<value>' } }).execute();
```
