-- Deploy: schemas/agentic_db_invites_public/procedures/submit_app_invite_code/procedure
...
CREATE FUNCTION agentic_db_invites_public.submit_app_invite_code(
  IN token text
) RETURNS boolean AS $_PGFN_$
DECLARE
  v_user agentic_db_users_public.users;
  v_email agentic_db_user_identifiers_public.emails;
  v_invite agentic_db_invites_public.app_invites;
BEGIN
  SELECT *
  ...
  END IF;
  SELECT *
  FROM agentic_db_invites_public.app_invites AS i
  WHERE
  ...
  IF v_email.email IS NOT NULL OR v_invite.multiple = false THEN
    UPDATE agentic_db_invites_public.app_invites SET
    invite_valid = false
    WHERE
      id = v_invite.id;
  END IF;
  ...
  WHERE
    id = v_invite.id;
  INSERT INTO agentic_db_invites_public.app_claimed_invites (
    sender_id,
    receiver_id,
    data
  )
  ...
  IF agentic_db_memberships_private.app_memberships_perm_check('send_approved_invites', v_invite.sender_id) IS TRUE THEN
