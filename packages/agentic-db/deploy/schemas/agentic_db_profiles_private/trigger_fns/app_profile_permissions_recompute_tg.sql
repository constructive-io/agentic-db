...
CREATE FUNCTION agentic_db_profiles_private.app_profile_permissions_recompute_tg() RETURNS TRIGGER AS $_PGFN_$
DECLARE
  v_profile_id uuid;
...
BEGIN
  ...
  FROM agentic_db_profiles_public.app_profile_permissions AS pp INNER JOIN agentic_db_permissions_public.app_permissions AS p ON p.id = pp.permission_id
  WHERE
    pp.profile_id = v_profile_id INTO v_permissions;
  UPDATE agentic_db_profiles_public.app_profiles SET
