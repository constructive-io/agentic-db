...
CREATE FUNCTION "agentic_db_profiles_private".org_profile_permissions_recompute_tg ()
  RETURNS TRIGGER
AS $CODEZ$
DECLARE
    v_profile_id uuid;
...
BEGIN
    ...
    INTO v_permissions
    FROM "agentic_db_profiles_public".org_profile_permissions pp
    JOIN "agentic_db_permissions_public".org_permissions p ON p.id = pp.permission_id
    WHERE pp.profile_id = v_profile_id;
    UPDATE "agentic_db_profiles_public".org_profiles
