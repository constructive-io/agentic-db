-- Deploy: schemas/agentic_db_limits_private/procedures/app_limits_check/procedure
...
CREATE FUNCTION agentic_db_limits_private.app_limits_check(
  IN limitname citext,
...
BEGIN
  SELECT
    EXISTS (SELECT 1
    FROM agentic_db_limits_public.app_limits
    WHERE
      name = app_limits_check.limitname AND actor_id = app_limits_check.user_id) INTO limit_exists;
  ...
  FROM agentic_db_limits_public.app_limits
  WHERE
    name = app_limits_check.limitname AND actor_id = app_limits_check.user_id INTO rec;
  IF rec.max >= (rec.num + app_limits_check.amount) THEN
    RETURN true;
  ELSE
    RETURN false;
  END IF;
