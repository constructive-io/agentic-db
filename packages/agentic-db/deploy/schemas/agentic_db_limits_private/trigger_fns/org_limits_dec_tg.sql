-- Deploy: schemas/agentic_db_limits_private/trigger_fns/org_limits_dec_tg
...
CREATE FUNCTION agentic_db_limits_private.org_limits_dec_tg() RETURNS TRIGGER AS $_PGFN_$
DECLARE
  actor_id uuid;
  limitname citext;
BEGIN
  ...
  ELSIF tg_nargs = 1 THEN
    limitname := (tg_argv)[0];
    PERFORM agentic_db_limits_private.org_limits_dec(limitname);
  ELSIF tg_nargs = 2 THEN
    limitname := (tg_argv)[0];
    EXECUTE pg_catalog.format('SELECT ($1).%s', (tg_argv)[1]) INTO actor_id USING OLD;
    PERFORM agentic_db_limits_private.org_limits_dec(limitname, actor_id);
  END IF;
  RETURN OLD;
END;
