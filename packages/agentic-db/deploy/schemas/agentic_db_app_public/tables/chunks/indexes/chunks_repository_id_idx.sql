-- Deploy: schemas/agentic_db_app_public/tables/chunks/indexes/chunks_repository_id_idx
-- made with <3 @ constructive.io

-- requires: schemas/agentic_db_app_public/schema
-- requires: schemas/agentic_db_app_public/tables/chunks/table
-- requires: schemas/agentic_db_app_public/tables/chunks/indexes/chunks_file_id_idx
-- requires: schemas/agentic_db_app_public/tables/chunks/columns/repository_id/column


CREATE INDEX chunks_repository_id_idx ON agentic_db_app_public.chunks USING BTREE ( repository_id );

