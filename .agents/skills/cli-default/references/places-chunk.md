...
agentic-db places-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db places-chunk list --limit 10 --after <cursor>
agentic-db places-chunk find-first --where.<field>.<op> <value>
agentic-db places-chunk search <query>
...
### List placesChunk records

```bash
agentic-db places-chunk list
```

### List placesChunk records with pagination

```bash
agentic-db places-chunk list --limit 10 --offset 0
```

### List placesChunk records with cursor pagination

```bash
agentic-db places-chunk list --limit 10 --after <cursor>
```

### Find first matching placesChunk

```bash
agentic-db places-chunk find-first --where.id.equalTo <value>
```

### List placesChunk records with field selection

```bash
agentic-db places-chunk list --select id,id
```

### List placesChunk records with filtering and ordering

```bash
agentic-db places-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
