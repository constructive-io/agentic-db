...
agentic-db skills-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db skills-chunk list --limit 10 --after <cursor>
agentic-db skills-chunk find-first --where.<field>.<op> <value>
agentic-db skills-chunk search <query>
...
### List skillsChunk records

```bash
agentic-db skills-chunk list
```

### List skillsChunk records with pagination

```bash
agentic-db skills-chunk list --limit 10 --offset 0
```

### List skillsChunk records with cursor pagination

```bash
agentic-db skills-chunk list --limit 10 --after <cursor>
```

### Find first matching skillsChunk

```bash
agentic-db skills-chunk find-first --where.id.equalTo <value>
```

### List skillsChunk records with field selection

```bash
agentic-db skills-chunk list --select id,id
```

### List skillsChunk records with filtering and ordering

```bash
agentic-db skills-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
