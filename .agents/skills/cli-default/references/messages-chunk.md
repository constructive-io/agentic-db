...
agentic-db messages-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db messages-chunk list --limit 10 --after <cursor>
agentic-db messages-chunk find-first --where.<field>.<op> <value>
agentic-db messages-chunk search <query>
...
### List messagesChunk records

```bash
agentic-db messages-chunk list
```

### List messagesChunk records with pagination

```bash
agentic-db messages-chunk list --limit 10 --offset 0
```

### List messagesChunk records with cursor pagination

```bash
agentic-db messages-chunk list --limit 10 --after <cursor>
```

### Find first matching messagesChunk

```bash
agentic-db messages-chunk find-first --where.id.equalTo <value>
```

### List messagesChunk records with field selection

```bash
agentic-db messages-chunk list --select id,id
```

### List messagesChunk records with filtering and ordering

```bash
agentic-db messages-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
