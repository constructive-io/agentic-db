...
agentic-db projects-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db projects-chunk list --limit 10 --after <cursor>
agentic-db projects-chunk find-first --where.<field>.<op> <value>
agentic-db projects-chunk search <query>
...
### List projectsChunk records

```bash
agentic-db projects-chunk list
```

### List projectsChunk records with pagination

```bash
agentic-db projects-chunk list --limit 10 --offset 0
```

### List projectsChunk records with cursor pagination

```bash
agentic-db projects-chunk list --limit 10 --after <cursor>
```

### Find first matching projectsChunk

```bash
agentic-db projects-chunk find-first --where.id.equalTo <value>
```

### List projectsChunk records with field selection

```bash
agentic-db projects-chunk list --select id,id
```

### List projectsChunk records with filtering and ordering

```bash
agentic-db projects-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
