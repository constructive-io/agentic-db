...
agentic-db runtime-states-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db runtime-states-chunk list --limit 10 --after <cursor>
agentic-db runtime-states-chunk find-first --where.<field>.<op> <value>
agentic-db runtime-states-chunk search <query>
...
### List runtimeStatesChunk records

```bash
agentic-db runtime-states-chunk list
```

### List runtimeStatesChunk records with pagination

```bash
agentic-db runtime-states-chunk list --limit 10 --offset 0
```

### List runtimeStatesChunk records with cursor pagination

```bash
agentic-db runtime-states-chunk list --limit 10 --after <cursor>
```

### Find first matching runtimeStatesChunk

```bash
agentic-db runtime-states-chunk find-first --where.id.equalTo <value>
```

### List runtimeStatesChunk records with field selection

```bash
agentic-db runtime-states-chunk list --select id,id
```

### List runtimeStatesChunk records with filtering and ordering

```bash
agentic-db runtime-states-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
