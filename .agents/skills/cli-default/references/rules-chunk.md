...
agentic-db rules-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db rules-chunk list --limit 10 --after <cursor>
agentic-db rules-chunk find-first --where.<field>.<op> <value>
agentic-db rules-chunk search <query>
...
### List rulesChunk records

```bash
agentic-db rules-chunk list
```

### List rulesChunk records with pagination

```bash
agentic-db rules-chunk list --limit 10 --offset 0
```

### List rulesChunk records with cursor pagination

```bash
agentic-db rules-chunk list --limit 10 --after <cursor>
```

### Find first matching rulesChunk

```bash
agentic-db rules-chunk find-first --where.id.equalTo <value>
```

### List rulesChunk records with field selection

```bash
agentic-db rules-chunk list --select id,id
```

### List rulesChunk records with filtering and ordering

```bash
agentic-db rules-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
