...
agentic-db tasks-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db tasks-chunk list --limit 10 --after <cursor>
agentic-db tasks-chunk find-first --where.<field>.<op> <value>
agentic-db tasks-chunk search <query>
...
### List tasksChunk records

```bash
agentic-db tasks-chunk list
```

### List tasksChunk records with pagination

```bash
agentic-db tasks-chunk list --limit 10 --offset 0
```

### List tasksChunk records with cursor pagination

```bash
agentic-db tasks-chunk list --limit 10 --after <cursor>
```

### Find first matching tasksChunk

```bash
agentic-db tasks-chunk find-first --where.id.equalTo <value>
```

### List tasksChunk records with field selection

```bash
agentic-db tasks-chunk list --select id,id
```

### List tasksChunk records with filtering and ordering

```bash
agentic-db tasks-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
