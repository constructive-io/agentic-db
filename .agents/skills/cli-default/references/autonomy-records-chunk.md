...
agentic-db autonomy-records-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db autonomy-records-chunk list --limit 10 --after <cursor>
agentic-db autonomy-records-chunk find-first --where.<field>.<op> <value>
agentic-db autonomy-records-chunk search <query>
...
### List autonomyRecordsChunk records

```bash
agentic-db autonomy-records-chunk list
```

### List autonomyRecordsChunk records with pagination

```bash
agentic-db autonomy-records-chunk list --limit 10 --offset 0
```

### List autonomyRecordsChunk records with cursor pagination

```bash
agentic-db autonomy-records-chunk list --limit 10 --after <cursor>
```

### Find first matching autonomyRecordsChunk

```bash
agentic-db autonomy-records-chunk find-first --where.id.equalTo <value>
```

### List autonomyRecordsChunk records with field selection

```bash
agentic-db autonomy-records-chunk list --select id,id
```

### List autonomyRecordsChunk records with filtering and ordering

```bash
agentic-db autonomy-records-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
