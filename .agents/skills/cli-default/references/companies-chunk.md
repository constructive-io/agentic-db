...
agentic-db companies-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db companies-chunk list --limit 10 --after <cursor>
agentic-db companies-chunk find-first --where.<field>.<op> <value>
agentic-db companies-chunk search <query>
agentic-db companies-chunk get --id <UUID>
agentic-db companies-chunk create --companiesId <UUID> --content <String> [--chunkIndex <Int>] [--embedding <Vector>] [--metadata <JSON>]
agentic-db companies-chunk update --id <UUID> [--companiesId <UUID>] [--content <String>] [--chunkIndex <Int>] [--embedding <Vector>] [--metadata <JSON>]
agentic-db companies-chunk delete --id <UUID>
```

## Examples

### List companiesChunk records

```bash
agentic-db companies-chunk list
```

### List companiesChunk records with pagination

```bash
agentic-db companies-chunk list --limit 10 --offset 0
```

### List companiesChunk records with cursor pagination

```bash
agentic-db companies-chunk list --limit 10 --after <cursor>
```

### Find first matching companiesChunk

```bash
agentic-db companies-chunk find-first --where.id.equalTo <value>
```

### List companiesChunk records with field selection

```bash
agentic-db companies-chunk list --select id,id
```

### List companiesChunk records with filtering and ordering

```bash
agentic-db companies-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
