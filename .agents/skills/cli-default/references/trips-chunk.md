...
agentic-db trips-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db trips-chunk list --limit 10 --after <cursor>
agentic-db trips-chunk find-first --where.<field>.<op> <value>
agentic-db trips-chunk search <query>
...
### List tripsChunk records

```bash
agentic-db trips-chunk list
```

### List tripsChunk records with pagination

```bash
agentic-db trips-chunk list --limit 10 --offset 0
```

### List tripsChunk records with cursor pagination

```bash
agentic-db trips-chunk list --limit 10 --after <cursor>
```

### Find first matching tripsChunk

```bash
agentic-db trips-chunk find-first --where.id.equalTo <value>
```

### List tripsChunk records with field selection

```bash
agentic-db trips-chunk list --select id,id
```

### List tripsChunk records with filtering and ordering

```bash
agentic-db trips-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
