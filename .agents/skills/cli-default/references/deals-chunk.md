...
agentic-db deals-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db deals-chunk list --limit 10 --after <cursor>
agentic-db deals-chunk find-first --where.<field>.<op> <value>
agentic-db deals-chunk search <query>
...
### List dealsChunk records

```bash
agentic-db deals-chunk list
```

### List dealsChunk records with pagination

```bash
agentic-db deals-chunk list --limit 10 --offset 0
```

### List dealsChunk records with cursor pagination

```bash
agentic-db deals-chunk list --limit 10 --after <cursor>
```

### Find first matching dealsChunk

```bash
agentic-db deals-chunk find-first --where.id.equalTo <value>
```

### List dealsChunk records with field selection

```bash
agentic-db deals-chunk list --select id,id
```

### List dealsChunk records with filtering and ordering

```bash
agentic-db deals-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
