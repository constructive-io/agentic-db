...
agentic-db events-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db events-chunk list --limit 10 --after <cursor>
agentic-db events-chunk find-first --where.<field>.<op> <value>
agentic-db events-chunk search <query>
...
### List eventsChunk records

```bash
agentic-db events-chunk list
```

### List eventsChunk records with pagination

```bash
agentic-db events-chunk list --limit 10 --offset 0
```

### List eventsChunk records with cursor pagination

```bash
agentic-db events-chunk list --limit 10 --after <cursor>
```

### Find first matching eventsChunk

```bash
agentic-db events-chunk find-first --where.id.equalTo <value>
```

### List eventsChunk records with field selection

```bash
agentic-db events-chunk list --select id,id
```

### List eventsChunk records with filtering and ordering

```bash
agentic-db events-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
