...
agentic-db runtime-logs-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db runtime-logs-chunk list --limit 10 --after <cursor>
agentic-db runtime-logs-chunk find-first --where.<field>.<op> <value>
agentic-db runtime-logs-chunk search <query>
...
### List runtimeLogsChunk records

```bash
agentic-db runtime-logs-chunk list
```

### List runtimeLogsChunk records with pagination

```bash
agentic-db runtime-logs-chunk list --limit 10 --offset 0
```

### List runtimeLogsChunk records with cursor pagination

```bash
agentic-db runtime-logs-chunk list --limit 10 --after <cursor>
```

### Find first matching runtimeLogsChunk

```bash
agentic-db runtime-logs-chunk find-first --where.id.equalTo <value>
```

### List runtimeLogsChunk records with field selection

```bash
agentic-db runtime-logs-chunk list --select id,id
```

### List runtimeLogsChunk records with filtering and ordering

```bash
agentic-db runtime-logs-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
