...
agentic-db venues-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db venues-chunk list --limit 10 --after <cursor>
agentic-db venues-chunk find-first --where.<field>.<op> <value>
agentic-db venues-chunk search <query>
...
### List venuesChunk records

```bash
agentic-db venues-chunk list
```

### List venuesChunk records with pagination

```bash
agentic-db venues-chunk list --limit 10 --offset 0
```

### List venuesChunk records with cursor pagination

```bash
agentic-db venues-chunk list --limit 10 --after <cursor>
```

### Find first matching venuesChunk

```bash
agentic-db venues-chunk find-first --where.id.equalTo <value>
```

### List venuesChunk records with field selection

```bash
agentic-db venues-chunk list --select id,id
```

### List venuesChunk records with filtering and ordering

```bash
agentic-db venues-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
