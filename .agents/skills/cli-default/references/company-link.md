...
agentic-db company-link list --where.<field>.<op> <value> --orderBy <values>
agentic-db company-link list --limit 10 --after <cursor>
...
agentic-db company-link create --url <String> --companyId <UUID> [--title <String>] [--embedding <Vector>] [--embeddingUpdatedAt <Datetime>]
agentic-db company-link update --id <UUID> [--title <String>] [--url <String>] [--embedding <Vector>] [--embeddingUpdatedAt <Datetime>] [--companyId <UUID>]
agentic-db company-link delete --id <UUID>
```

## Examples

### List companyLink records

```bash
agentic-db company-link list
```

### List companyLink records with pagination

```bash
agentic-db company-link list --limit 10 --offset 0
```

### List companyLink records with cursor pagination

```bash
agentic-db company-link list --limit 10 --after <cursor>
```

### Find first matching companyLink

```bash
agentic-db company-link find-first --where.id.equalTo <value>
```

### List companyLink records with field selection

```bash
agentic-db company-link list --select id,id
```

### List companyLink records with filtering and ordering

```bash
agentic-db company-link list --where.id.equalTo <value> --orderBy ID_ASC
```
