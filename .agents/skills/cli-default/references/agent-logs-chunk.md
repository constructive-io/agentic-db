...
agentic-db agent-logs-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db agent-logs-chunk list --limit 10 --after <cursor>
agentic-db agent-logs-chunk find-first --where.<field>.<op> <value>
agentic-db agent-logs-chunk search <query>
...
### List agentLogsChunk records

```bash
agentic-db agent-logs-chunk list
```

### List agentLogsChunk records with pagination

```bash
agentic-db agent-logs-chunk list --limit 10 --offset 0
```

### List agentLogsChunk records with cursor pagination

```bash
agentic-db agent-logs-chunk list --limit 10 --after <cursor>
```

### Find first matching agentLogsChunk

```bash
agentic-db agent-logs-chunk find-first --where.id.equalTo <value>
```

### List agentLogsChunk records with field selection

```bash
agentic-db agent-logs-chunk list --select id,id
```

### List agentLogsChunk records with filtering and ordering

```bash
agentic-db agent-logs-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
