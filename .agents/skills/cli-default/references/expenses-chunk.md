...
agentic-db expenses-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db expenses-chunk list --limit 10 --after <cursor>
agentic-db expenses-chunk find-first --where.<field>.<op> <value>
agentic-db expenses-chunk search <query>
...
### List expensesChunk records

```bash
agentic-db expenses-chunk list
```

### List expensesChunk records with pagination

```bash
agentic-db expenses-chunk list --limit 10 --offset 0
```

### List expensesChunk records with cursor pagination

```bash
agentic-db expenses-chunk list --limit 10 --after <cursor>
```

### Find first matching expensesChunk

```bash
agentic-db expenses-chunk find-first --where.id.equalTo <value>
```

### List expensesChunk records with field selection

```bash
agentic-db expenses-chunk list --select id,id
```

### List expensesChunk records with filtering and ordering

```bash
agentic-db expenses-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
