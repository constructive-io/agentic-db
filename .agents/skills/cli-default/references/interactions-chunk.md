...
agentic-db interactions-chunk list --where.<field>.<op> <value> --orderBy <values>
agentic-db interactions-chunk list --limit 10 --after <cursor>
agentic-db interactions-chunk find-first --where.<field>.<op> <value>
agentic-db interactions-chunk search <query>
...
### List interactionsChunk records

```bash
agentic-db interactions-chunk list
```

### List interactionsChunk records with pagination

```bash
agentic-db interactions-chunk list --limit 10 --offset 0
```

### List interactionsChunk records with cursor pagination

```bash
agentic-db interactions-chunk list --limit 10 --after <cursor>
```

### Find first matching interactionsChunk

```bash
agentic-db interactions-chunk find-first --where.id.equalTo <value>
```

### List interactionsChunk records with field selection

```bash
agentic-db interactions-chunk list --select id,id
```

### List interactionsChunk records with filtering and ordering

```bash
agentic-db interactions-chunk list --where.id.equalTo <value> --orderBy ID_ASC
```
