...
db.calendarAttendee.create({ data: { contactId: '<UUID>', responseStatus: '<String>', role: '<String>', calendarEventId: '<UUID>' }, select: { id: true } }).execute()
db.calendarAttendee.update({ where: { id: '<UUID>' }, data: { contactId: '<UUID>' }, select: { id: true } }).execute()
...
const item = await db.calendarAttendee.create({
  data: { contactId: '<UUID>', responseStatus: '<String>', role: '<String>', calendarEventId: '<UUID>' },
  select: { id: true }
}).execute();
```
