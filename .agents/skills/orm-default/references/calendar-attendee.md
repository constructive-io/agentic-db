...
db.calendarAttendee.create({ data: { entityId: '<UUID>', calendarEventId: '<UUID>', contactId: '<UUID>', responseStatus: '<String>', role: '<String>' }, select: { id: true } }).execute()
...
const item = await db.calendarAttendee.create({
  data: { entityId: '<UUID>', calendarEventId: '<UUID>', contactId: '<UUID>', responseStatus: '<String>', role: '<String>' },
  select: { id: true }
}).execute();
```
